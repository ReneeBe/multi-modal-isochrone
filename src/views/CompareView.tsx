        ...
        <div className="pointer-events-auto flex flex-col gap-3 max-w-md">
          <div className="flex flex-col gap-1">
            <h1 className="text-lg md:text-xl font-bold tracking-tight drop-shadow-lg text-zinc-900 dark:text-white">
              Walking vs biking, side by side
            </h1>
            <p className="text-xs drop-shadow text-zinc-700 dark:text-zinc-400">
              Same pin, two routing engines. Watch the bike polygons stretch on flat ground and shrink up steep climbs.
            </p>
          </div>
...
function MapPanel({
  ...
  return (
    <div className="relative h-full overflow-hidden">
      ...
      {/* Mode label, big pill near the divider so the search panel
          on the left and the theme toggle on the right can't cover it */}
      <div
        className={`absolute top-4 ${isLeft ? 'right-6' : 'left-6'} pointer-events-none z-20`}
      >
        <div className="px-4 py-2 rounded-full backdrop-blur border-2 text-sm font-bold tracking-widest uppercase bg-white/90 border-fuchsia-500/70 text-fuchsia-700 dark:bg-zinc-900/90 dark:border-fuchsia-400/70 dark:text-fuchsia-300 shadow-lg">
          {label}
        </div>
      </div>
